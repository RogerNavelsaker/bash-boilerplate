...
readonly __template_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -f "${__template_dir}/core.sh" ]]; then
    source "${__template_dir}/core.sh"
else
    echo "Error: core.sh not found at ${__template_dir}/core.sh" >&2
    exit 1
fi
