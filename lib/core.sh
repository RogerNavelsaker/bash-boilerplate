...
# Bash Boilerplate Core Library (lib/core.sh)
# 
# Usage: source "./lib/core.sh"
# 
# This file contains the safety flags, magic variables, logging,
# and utility functions for the modular version of the boilerplate.
...
log() {
    ...
    if [[ -n "${log_msg:-}" ]]; then
        echo -e "${log_msg}" >&2
        [[ -n "${LOG_FILE}" ]] && echo -e "${log_msg}" | sed 's/\x1b\[[0-9;]*m//g' >> "${LOG_FILE}"
    fi
}
...
check_dependencies() {
    for cmd in "$@"; do
        command -v "${cmd}" >/dev/null 2>&1 || die "Required dependency not found: ${cmd}"
    done
}

is_app_installed() { command -v "${1}" >/dev/null 2>&1; }
is_cmd() { is_app_installed "$1"; }
...
retry() {
    ...
    local -i attempt=1
    until eval "${cmd}"; do
        [[ "${attempt}" -eq "${max_attempts}" ]] && log ERROR "Command failed after ${max_attempts} attempts: ${cmd}" && return 1
...
is_empty() {
    local -r path="${1:-}"
    if [[ -d "${path}" ]]; then [[ -z "$(ls -A "${path}")" ]]; elif [[ -f "${path}" ]]; then [[ ! -s "${path}" ]]; else [[ -z "${path}" ]]; fi
}
...
is_online() {
    for host in "8.8.8.8" "1.1.1.1" "google.com"; do ping -c 1 -W 1 "$host" >/dev/null 2>&1 && return 0; done
    return 1
}
...
check_bash_version() {
    [[ "${BASH_VERSINFO[0]}" -lt "${1:-4}" ]] && die "This script requires Bash ${1:-4} or higher. Current version: ${BASH_VERSION}"
}

timer_start() { __timer_start=$(date +%s); }
timer_stop() {
    local diff=$(( $(date +%s) - __timer_start ))
...
abs_path() {
    local path="${1}"
    if [[ -d "${path}" ]]; then (cd "${path}" && pwd); elif [[ -f "${path}" ]]; then (cd "$(dirname "${path}")" && pwd)/$(basename "${path}"); else echo "${path}"; fi
}

get_ext() { local filename=$(basename -- "$1"); echo "${filename##*.}"; }

random_string() {
    local len="${1:-16}"
    if is_cmd tr && [[ -r /dev/urandom ]]; then LC_ALL=C tr -dc 'a-zA-Z0-9' < /dev/urandom | head -c "${len}"; echo; else printf "%s" "$RANDOM$RANDOM$RANDOM" | head -c "${len}"; echo; fi
}
...
lock() {
    local l="${1:-/tmp/${__base}.lock}"
    if [[ -e "${l}" ]]; then local p=$(cat "${l}"); kill -0 "${p}" 2>/dev/null && die "Script already running (PID: ${p})"; fi
...
load_env() {
    ...
    while IFS='=' read -r k v || [[ -n "${k}" ]]; do
        [[ "${k}" =~ ^#.*$ || -z "${k}" ]] && continue
        k=$(echo "${k}" | tr -d '[:space:]')
        v=$(echo "${v}" | tr -d '[:space:]' | sed "s/^'//;s/'$//;s/^\"//;s/\"$//")
        export "${k}=${v}"
    done < "${f}"
}

wait_for_url() {
    local u="${1}" t="${2:-30}" c=0
    until quiet curl -s --head --request GET "${u}"; do
        sleep 1; ((c++))
        [[ "${c}" -ge "${t}" ]] && return 1
    done
    return 0
